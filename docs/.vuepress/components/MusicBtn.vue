<template>
  ...
  <div class="music-box" v-if="curMusic.songname" @mouseover="onmouseover" @mouseleave="onmouseleave">
    ...
    <div v-if="tooltip" class="tooltip-box">
      <div class="arrow"></div>
      <div class="text">{{ curMusic.songname  }}</div>
    </div>
  </div>
  <!-- 音乐播放器 -->
  <el-drawer
  ...
  >
    <template #header>
      <div class="header-box">
        ...
        <div class="play-music">正在播放 <span v-if="curMusic" class="cur-name">{{ curMusic.songname  }} - {{ curMusic.author }}</span></div>
      </div>
    </template>
...
export default {
  ...
  setup(props, context) {
    ...
    const getcurMusic = (value) => {
      value ? curMusic.value = value : curMusic.value = { songname: '' };
    }
    onMounted(() => {
      getcurMusic();
    })
...
.tooltip-box {
  ...
  width: 0px;
    .text {
      ...
      padding: 2px 12px;
      border-radius: 5px;
      background: linear-gradient(90deg, #2f85ef, #80e2fc);
      text-overflow: ellipsis;
      overflow: hidden;
      max-width: 200px;
    }
    .arrow {
      ...
      position: absolute;
      top: 31px;
      background: conic-gradient(from 90deg at 0% 0, #2f85ef 0, #4092f5 45deg, transparent 45.1deg);
      left: -36px;
